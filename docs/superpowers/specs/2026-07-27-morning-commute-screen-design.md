...
class Departure:
    ...
    platform: str            # locationMetadata.platform.actual ?? .planned
    cancelled: bool          # temporalData.departure.isCancelled
    lateness: int            # realtimeAdvertisedLateness, minutes (0 if null)
    destination: str
```
...
exactly its error. It is called out in the README.
